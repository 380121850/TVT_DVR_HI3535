...
struct ata_port *SynoEunitFindMaster(struct ata_port *ap)
{
	...
	iAtaPrintIdMax = atomic_read(&ata_print_id) + 1; /* ata_print_id is type "atomic_t", it will happen invalid operands errors while comparing to type "int" */
...
#ifdef SYNO_OLD_INTERFACE /* disable on 3.10.x */
