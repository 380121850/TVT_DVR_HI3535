/*
 * Source code of this file largely depends on Hi3535
 * architecture.
 */
...
#define PCIE_INT_A			80
#define PCIE_INT_B			81
#define PCIE_INT_C			82
#define PCIE_INT_D			83

#define PCIE_DMA_LOCAL_IRQ		84

#define PCIE_LOCAL_CFG			0x20800000
...
#define HI3535_DEBUG            4
#define HI3535_INFO             3
#define HI3535_ERR              2
#define HI3535_FATAL            1
#define HI3535_CURRENT_LEVEL    2
#define HI3535_TRACE(level, s, params...)   do{ \
	        if(level <= HI3535_CURRENT_LEVEL) \
...
static unsigned int get_local_slot_number(void)
{
	...
	slot &= 0x1f;
	if (0 == slot)
		HI3535_TRACE(HI3535_ERR, "invalid local slot"
				" number[0x%x]!", slot);
#endif
...
static int init_hidev(struct hi35xx_dev *hi_dev)
{
	...
	hi_dev->pci_bar3_virt = (unsigned int)ioremap(hi_dev->bar3, 0x1000);
	if (!hi_dev->pci_bar3_virt) {
		HI3535_TRACE(HI3535_ERR, "IO remap for dev[%d] bar3 failed!",
				hi_dev->slot_index);
		return -ENOMEM;
	}
...
static void __start_dma_task(void *task)
{
	...
	if (new_task->dir == PCI_DMA_WRITE) {
		...
		HI3535_TRACE(HI3535_ERR, "Wrong dma task data![dir 0x%x]!",
				new_task->dir);
		HI3535_TRACE(HI3535_ERR, "Start_dma_task failed!");
	}
...
static int clear_dma_read_local_irq(unsigned int controller)
{
	...
	HI3535_TRACE(HI3535_INFO, "PCIe DMA irq status: read[0x%x]", read_status);

	if (0x0 == read_status) {
		HI3535_TRACE(HI3535_INFO, "No PCIe DMA read irq triggerred!");
		return 0;
	}

	if (unlikely(DMA_ABORT_INTERRUPT_BIT & read_status)) {
		HI3535_TRACE(HI3535_ERR, "DMA read abort !!!");
		ret = 2;
	}
...
static int clear_dma_write_local_irq(unsigned int controller)
{
	...
	HI3535_TRACE(HI3535_INFO, "PCIe DMA irq status: write[0x%x]", write_status);
	if (0x0 == write_status) {
		HI3535_TRACE(HI3535_INFO, "No PCIe DMA write irq triggerred!");
		return 0;
	}
	if (unlikely(DMA_ABORT_INTERRUPT_BIT & write_status)) {
		HI3535_TRACE(HI3535_ERR, "DMA write abort !!!");
		ret = 2;
	}
...
static int request_dma_resource(irqreturn_t (*handler)(int irq, void *id))
{
	...
	if (ret) {
		HI3535_TRACE(HI3535_ERR, "request PCIe DMA irq failed!");
	}
...
static int request_message_irq(irqreturn_t (*handler)(int irq, void *id))
{
			...
			&& s_pcie_opt->local_controller[0]->int_d)
	{
		...
		if (ret) {
			HI3535_TRACE(HI3535_ERR, "Request PCIE INT-A " 
					"[%d] failed[%d].",
		...
		if (ret) {
			HI3535_TRACE(HI3535_ERR, "Request PCIE INT-B " 
					"[%d] failed[%d].",
					s_pcie_opt->local_controller[0]->int_b,
					ret);
			goto pcie_int_b_err;
		...
		if (ret) {
			HI3535_TRACE(HI3535_ERR, "Request PCIE INT-C " 
					"[%d] failed[%d].",
					s_pcie_opt->local_controller[0]->int_c,
					ret);
			goto pcie_int_c_err;
		...
		if (ret) {
			HI3535_TRACE(HI3535_ERR, "Request PCIE INT-D " 
					"[%d] failed[%d].",
					s_pcie_opt->local_controller[0]->int_d,
					ret);
			goto pcie_int_d_err;
		...
		HI3535_TRACE(HI3535_ERR, "PCIE INT irq number err "
				"[%d, %d, %d, %d]",
				s_pcie_opt->local_controller[0]->int_a,
				s_pcie_opt->local_controller[0]->int_b,
				s_pcie_opt->local_controller[0]->int_c,
...
pcie_int_b_err:
	...
	if (ret) {
		HI3535_TRACE(HI3535_ERR, "Request glb soft "
				"irq[%d] failed[%d]!",
...
static int clear_pcie_int(struct hi35xx_dev *hi_dev)
{
	...
	if (!hi_dev) {
		HI3535_TRACE(HI3535_ERR, "Clear PCIe INT irq err, "
				"dev does not exist!");
		return -1;
	}

	status = readl(hi_dev->pci_bar3_virt + MISC_CTRL67);

	if (status & PCIE_INT_X_BIT) {
		status &= (~PCIE_INT_X_BIT);
		writel(status, hi_dev->pci_bar3_virt + MISC_CTRL67);
	} else {
...
{
	if (!hi_dev) {
		HI3535_TRACE(HI3535_ERR, "global soft irq to slave failed,"
...
#if defined SLV_ARCH_HI3535
	val = readl(s_pcie_opt->misc_reg_virt + MISC_CTRL67);
	val |= PCIE_INT_X_BIT;
	writel(val, s_pcie_opt->misc_reg_virt + MISC_CTRL67);
#elif defined SLV_ARCH_HI3531
#elif defined SLV_ARCH_HI3532
#endif
}
		...
		unsigned int index)
{
#ifdef __IS_PCI_HOST__
	if (!hi_dev) {
		HI3535_TRACE(HI3535_ERR, "move pcie pf window failed,"
				" hi_dev is NULL!");
		return;
	}
	...
	writel(0x80000000 | ATU_REG_INDEX_BIT(index),
			local_config_virt + ATU_VIEW_PORT);
	...
	writel(0xc0000000 | ATU_BAR_NUM_BIT(index),
			local_config_virt + ATU_REGION_CONTROL2);
#endif
}
		...
		unsigned int index)
{
	...
	writel(s_pcie_opt->local_controller[0]->win_base[index],
			local_config_virt + ATU_BASE_LOWER);
	writel(0x0, local_config_virt + ATU_BASE_UPPER);
	writel(s_pcie_opt->local_controller[0]->win_base[index]
...
static int host_pcie_controller_init(void)
{
	...
	HI3535_TRACE(HI3535_ERR, "No PCIe controller enabled in host!");
	/* quit here */
	return -1;
#endif
	...
	if (NULL == s_pcie_opt->local_controller[0]) {
		HI3535_TRACE(HI3535_ERR, "kmalloc for pcie failed!\n");
		return -ENOMEM;
	}
	...
	HI3535_TRACE(HI3535_INFO, "PCIE INT irq number"
			"[%d, %d, %d, %d]",
			s_pcie_opt->local_controller[0]->int_a,
			s_pcie_opt->local_controller[0]->int_b,
			s_pcie_opt->local_controller[0]->int_c,
	...
	if (!s_pcie_opt->local_controller[0]->config_virt) {
		HI3535_TRACE(HI3535_ERR, "ioremap for pcie_cfg failed!");
...
static int host_handshake_step1(struct hi35xx_dev *hi_dev)
{
	...
	while (1) {
		...
		if (0 == check_remote_timeout--) {
			HI3535_TRACE(HI3535_ERR, "Handshake timeout[device %d]",
					hi_dev->slot_index);
			return -1;
		}
...
static int slv_handshake_step0(void)
{
	...
	while (1) {
		...
		HI3535_TRACE(HI3535_INFO, "Host is not ready yet!");

		if (0 == check_remote_timeout--) {
			HI3535_TRACE(HI3535_ERR, "Handshake step0 timeout!");
			return -1;
		}
	...
	if (0 == s_pcie_opt->local_devid) {
		HI3535_TRACE(HI3535_ERR, "invalid local id[0],"
...
static int slv_pcie_controller_init(void)
{
	...
	s_pcie_opt->local_controller[0] = kmalloc(sizeof(struct pci_controller),GFP_KERNEL);
	if (NULL == s_pcie_opt->local_controller[0]) {
		HI3535_TRACE(HI3535_ERR, "kmalloc for pcie_controller%d failed!", controller_id);
		return -ENOMEM;
	}
	...
	s_pcie_opt->local_controller[0]->config_virt
		= (unsigned int)ioremap_nocache(s_pcie_opt->local_controller[0]->config_base,
				0x1000);
	if (!s_pcie_opt->local_controller[0]->config_virt) {
		HI3535_TRACE(HI3535_ERR, "ioremap for pcie%d_cfg failed!",
				controller_id);
		ret = -ENOMEM;
	...
	if (!s_pcie_opt->local_controller[0]->win_virt[0]) {
		HI3535_TRACE(HI3535_ERR, "ioremap for win0 failed!");
		ret = -ENOMEM;
		goto alloc_win0_err;
	}
	...
	if (!s_pcie_opt->local_controller[0]->win_virt[1]) {
		HI3535_TRACE(HI3535_ERR, "ioremap for win1 failed!");
		ret = -ENOMEM;
		goto alloc_win1_err;
	}
	...
	if (!s_pcie_opt->local_controller[0]->win_virt[2]) {
		HI3535_TRACE(HI3535_ERR, "ioremap for win2 failed!");
		ret = -ENOMEM;
		goto alloc_win2_err;
	}
...
int pci_arch_init(struct pci_operation *handler)
{
	...
	if (host_pcie_controller_init()) {
		HI3535_TRACE(HI3535_ERR, "Host PCIe controller init failed!");
		ret = -1;
	}
#else
	...
	if (slv_pcie_controller_init()) {
		HI3535_TRACE(HI3535_ERR, "Slave PCIe controller init failed!");
		return -1;
	}

	s_pcie_opt->sysctl_reg_virt = (unsigned int)ioremap_nocache(s_pcie_opt->sysctl_reg_phys, 0x1000);
	if (!s_pcie_opt->sysctl_reg_virt) {
		HI3535_TRACE(HI3535_ERR, "IO remap for SYS_CTRL registers failed!");
		ret = -ENOMEM;
		goto pcie_controller_init_err;
	}

	s_pcie_opt->misc_reg_virt = (unsigned int)ioremap_nocache(s_pcie_opt->misc_reg_phys, 0x1000);
	if (!s_pcie_opt->misc_reg_virt) {
		HI3535_TRACE(HI3535_ERR, "IO remap for MISC_CTRL registers failed!");
		ret = -ENOMEM;
		goto pcie_controller_init_err;
	}
