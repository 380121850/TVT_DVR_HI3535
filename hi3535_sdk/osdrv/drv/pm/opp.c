...
struct opp *opp_find_freq_ceil(struct device *dev, unsigned long *freq)
{
    ...
    if (!dev || !freq)
    {
        HI_TRACE("%s: Invalid argument freq=%p\n", __func__, freq);
...
int opp_add(struct device *dev, unsigned long freq, unsigned long u_volt)
{
    ...
    if (!new_opp)
    {
        HI_TRACE("%s: Unable to create new OPP node\n", __func__);
        return -ENOMEM;
    }
    ...
    if (IS_ERR(dev_opp))
    {
        ...
        if (!dev_opp)
        {
            ...
            kfree(new_opp);
            HI_TRACE("%s: Unable to create device OPP structure\n",
                       __func__);
            return -ENOMEM;
        }
                                ...
                                bool availability_req)
{
    ...
    if (!new_opp)
    {
        HI_TRACE("%s: Unable to create OPP\n", __func__);
        return -ENOMEM;
    }
    ...
    {
        r = PTR_ERR(dev_opp);
        HI_TRACE("%s: Device OPP not found (%d)\n", __func__, r);
        goto unlock;
    }
                           ...
                           struct cpufreq_frequency_table **table)
{
    ...
    {
        mutex_unlock(&dev_opp_list_lock);
        HI_TRACE("%s: Unable to allocate frequency table\n",
                   __func__);
        return -ENOMEM;
    }
                             ...
                             u32    opp_def_size)
{
    ...
    for (i = 0; i < opp_def_size; i++)
    {
        ...
        if (r)
        {
            HI_TRACE("%s: add OPP %ld failed for [%d] "
                      "result=%d\n",
        ...
        else
        {
            ...
            if (r)
            {
                HI_TRACE("%s: disable %ld failed for"
                          "[%d] result=%d\n",
                          __func__, opp_def->freq,
                          i, r);
            }
