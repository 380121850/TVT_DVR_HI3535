                             ...
                             unsigned int target_freq, unsigned int relation)
{
    ...
    if (!freq_table)
    {
        HI_TRACE("%s: cpu%d: no freq table!\n", __func__,
                  policy->cpu);
        return -EINVAL;
    }
    ...
    if (ret)
    {
        HI_TRACE("%s: cpu%d: no freq match for %d(ret=%d)\n",
...
static int hi_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
    ...
    if (result)
    {
        HI_TRACE("%s: cpu%d: failed creating freq table[%d]\n",
...
int __init hi_cpufreq_init(void)
{
    int ret;
    ...
    if (!ret)
    {
        ...
        if (t)
        {
            HI_TRACE("%s_init: platform_device_register failed\n",
                      __func__);
        }
        ...
        if (t)
        {
            HI_TRACE("%s_init: platform_driver_register failed\n",
                       __func__);
        }
    }
...
void __exit hi_cpufreq_exit(void)
{
    HI_TRACE("hi_cpufreq_exit\n");
