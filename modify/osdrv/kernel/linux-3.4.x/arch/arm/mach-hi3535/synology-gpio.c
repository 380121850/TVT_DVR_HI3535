...
//(*get_sys_temperature)(struct _SynoThermalTemp *);
int GetSysTemperature(void *p)
{
	printk("Lee DEBUG===GetSysTemperature\n");
    return 0;
}
EXPORT_SYMBOL(GetSysTemperature);
...
void synology_gpio_init(void)
{
	...
	printk("Lee DEBUG===FAKE GPIO INIT\n");
#else
