...
#define MTDPARTS_DEFAULT "mtdparts=hinand:1M(RedBoot),4M(zImage),6M(rd.gz),128K(vendor),128K(RedBoot+Config),9M(FIS+directory),-(user)"
#define DEFAULT_PART MTDPARTS_DEFAULT";hi_sfc:2M(boot)"
 ...
 *  Environment   Configuration
 ------------------------------------------------------------------------*/
/* only for pcie slave */
#define CONFIG_BOOTCOMMAND "mtdparts default;nand read 0x82000000 zImage;"\
                            "bootm 0x82000000"

#define CONFIG_BOOTDELAY	1
#define CONFIG_BOOTARGS "console=ttyS0,115200 mem=896M syno_hw_version=NVR216 "\
                         "syno_phys_memsize=1024 initrd=0x82000000 root=/dev/md0 rw init=/sbin/init "\
                         "sn=C7LWN09771 hd_power_on_seq=2 ihd_num=5 netif_num=1 flash_size=128 "MTDPARTS_DEFAULT
#define CONFIG_NETMASK  255.255.254.0       /* talk on MY local net */
#define CONFIG_IPADDR   192.168.50.86        /* static IP I currently own */
#define CONFIG_SERVERIP 192.168.50.118     /* current IP of tftp server ip */
...
#define CFG_DDR_SIZE		(1*1024*1024*1024)
